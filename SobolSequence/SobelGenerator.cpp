...
*/
void SobelGenerator::TransformDirectionIntoCurrentRaySpace(Vector &scatterDir, vector<Vector *> largeRayDirectionIDSpace
									,vector<Vector *> & resultRayDirectionInRaySpace)
{
	...
	for(int i = 0 ; i < outDistr ->N ; i++) {
		Vector *vec = new Vector();
		vec ->x = this ->GetPoint(i).x;
		vec ->y = this ->GetPoint(i).y;
		vec ->z = this ->GetPoint(i).z;
...
void SobelGenerator::TransformDirectionIntoCurrentRaySpace(Vector &scatterDir, vector<Vector *> & resultRayDirectionInRaySpace)
{
	Vector arbitrary(0.f, 1.f, 0.f);
	Vector x = Cross(scatterDir, arbitrary);
	Vector y = Cross(x, scatterDir);
...
Matrix4x4 * SobelGenerator::TransformVector(const Vector &z)
{
	Vector arbitrary(0.f, 1.f, 0.f);
	Vector x = Cross(z, arbitrary);
	Vector y = Cross(x, z);
	Matrix4x4 *M = new Matrix4x4(x.x, y.x, z.x, 0.f,
								 x.y, y.y, z.y, 0.f,
								 x.z, y.z, z.z, 0.f,
								 0.f, 0.f, 0.f, 1.0f);

	// new scatter Direction == SobelGeneratedVector * M;
	return M;
}
...
void SobelGenerator::PickRandomPoints(const RNG &rng, int nSamples, vector<int> &VectorIndices, vector<Matrix4x4 *> &transformationMatrices)
{
	for(int i = 0 ; i < nSamples ; i++) {
		...
		Matrix4x4 *matrix = TransformVector(vec);
...
For n=1, you get a distribution over the entire sphere. As n gets bigger, the distribution narrows dramatically, with few, if any, points far from the center.
		...
		*H *= H ->z * 2.0f;
		...
		//compute pdf.
		float pdf = (power+1) * pow(z, power) / ( 8.f * PI * H ->z );
		(pdfArray)[i] = pdf;
	}
